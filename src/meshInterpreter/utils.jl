...
function lagrangeCoefficient!(
    x::AbstractFloat,
    xigll::Vector{Float32},
    hlagx::Vector{Float32},
)
    hlagx .= 1.0f0
    for j = 1:length(xigll)
        for i = 1:length(xigll)
            if i != j
...
function cube2xyz(
    anchor_xyz::Array{Float32,2},
    uvw::Array{Float32,1},
    xyz::Array{Float32,1},
    DuvwDxyz::Array{Float32,2},
)
...
function xyz2cube_bounded(
    xyz_anchor::Array{Float32,2},
    xyz::Array{Float32,1},
)::Tuple{Array{Float32,1},AbstractFloat,Bool}
    niter = 5
    uvw = zeros(Float32, 3)
