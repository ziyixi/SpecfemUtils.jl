...
mutable struct Sem_mesh_data
    ...
    Sem_mesh_data(nspec::Integer, nglob::Integer) = new(nspec, nglob, zeros(Float32, 3, nglob), zeros(Int32, NGLLX, NGLLY, NGLLZ, nspec), zeros(Int32, nspec), zeros(Bool, nspec))
end
...
mutable struct Sem_mesh_location
    ...
    Sem_mesh_location() = new(-1, -1, zeros(Float32, 3), Inf32, zeros(Float32, NGLLX, NGLLY, NGLLZ))
end
...
struct CmdArgs
    ...
    CmdArgs(parsed_dict::Dict) = begin
        ...
        lon1, lon2, lat1, lat2, dep1, dep2 = [parse(Float32,i) for i in split(parsed_dict["region"], "/")]
        lonnpts, latnpts, vnpts = [parse(Int32,i) for i in split(parsed_dict["npts"], "/")]
        lonnproc, latnproc = [parse(Int32,i) for i in split(parsed_dict["process"], "/")]
...
const specfem_ellip = Geodesy.Ellipsoid(a="6378137.0", f_inv="299.8", name=:specfem_ellip)
